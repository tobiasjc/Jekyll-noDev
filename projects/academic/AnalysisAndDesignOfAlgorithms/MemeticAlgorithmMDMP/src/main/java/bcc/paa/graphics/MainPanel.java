...
public class MainPanel extends JPanel {
	...
	public MainPanel(Device[] devices, Tour route, int sourcePoint, double maxValue) {
	...
	public synchronized void paintComponent(Graphics g) {
		...
		// draw graph things
		for (int idx = 0; idx < this.devices.length; idx++) {
			// get coordinates scaled
			Device d = this.devices[idx];
			double x = d.x * scaleX;
			double y = d.y * scaleY;
		...
		// draw route things
		int size = this.route.filled;
		for (int i = 0; i < size; i++) {
			// get scaled points coordinates at the center of the ellipses
			Device p1 = this.route.path[i];
			Device p2 = this.route.path[(i + 1) % size];
